...
export function Preloader({ onComplete, name = "MUHAMMAD HAMZA" }: { onComplete: () => void, name?: string }) {
      ...
      {!isExiting && (
        ...
        >
          ...
          <div className="absolute inset-0 flex flex-col items-center justify-center pointer-events-none z-10 px-6 text-center">
            ...
            >
              <h1 className={`text-2xl min-[400px]:text-3xl sm:text-4xl md:text-7xl font-bold tracking-[0.15em] md:tracking-[0.3em] mb-4 drop-shadow-[0_0_25px_rgba(255,255,255,0.4)] overflow-hidden flex flex-wrap justify-center ${mode === "sakura" ? "text-pink-600 dark:text-pink-300" : "text-orange-700 dark:text-orange-400"}`}>
                  ...
                  >
                    {word.split("").map((letter, letterIndex) => (
                      <motion.span
                        ...
                        transition={{
                          delay: 0.5 + wordIndex * 0.1 + letterIndex * 0.03, // Extra 0.5s delay so it plays smoothly with preloader fade-in
                          type: "spring",
                          stiffness: 150,
                          damping: 25,
                        }}
